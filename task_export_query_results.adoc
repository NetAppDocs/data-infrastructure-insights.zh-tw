...
summary: 使用非同步處理將查詢結果、儀表板表格、活動網頁和其他資料匯出至 .CSV 檔案，以便進行外部分析、報告以及與其他應用程式整合。 
...
[role="lead"]
將查詢、儀表板表格元件和活動網頁表格中的資料匯出為 .CSV 檔案，以便進行外部分析、報告以及與其他應用程式整合。非同步匯出程序可讓您在背景處理大型資料集的同時繼續工作。
