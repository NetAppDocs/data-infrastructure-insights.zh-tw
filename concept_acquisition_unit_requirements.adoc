...
這台電腦不應執行任何其他應用程式層級軟體。建議使用專用伺服器。

如果您正在執行 SELinux，建議在擷取單元系統上執行下列命令：
...
對於 Linux 系統，磁碟空間應按以下方式分配：

* /opt/netapp 10 GB （大型環境為 20 GB）
* /var/log/netapp 40 GB （大型環境為 80 GB）
* 安裝期間 /tmp 至少需要 1 GB 可用空間


您的檔案系統可能會使用一些空間作為其額外負荷。這些是安裝前應 _可用_ 的最小空間。
| 50GB 
