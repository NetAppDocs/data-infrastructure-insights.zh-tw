...
summary: 透過 API 直接存取報表資料庫，繞過 Cognos 實作自訂整合、自動報表和進階資料分析工作流程。 
...
[role="lead"]
透過符合 OData v4 標準的 API 直接存取 Data Infrastructure Insights 報告資料庫，繞過 Cognos 介面以獲得更大的靈活度。這能為需要以程式設計方式存取其基礎架構資料的使用者，實現自訂整合、自動化報告工作流程與進階資料分析。
...
Data Infrastructure Insights 報告 API 遵循 link:https://www.odata.org/["OData v4"] （Open Data Protocol）標準來查詢報告資料庫。若要深入瞭解，請查看 OData 上的 link:https://www.odata.org/getting-started/basic-tutorial/["本教程"]。
