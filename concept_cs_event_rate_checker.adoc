...
summary: 使用 SVM 事件速率檢查器測量 NFS/SMB 事件速率，並在部署工作負載安全資料收集器之前確定最佳代理程式大小。 
...
[role="lead"]
在部署資料收集器之前，請透過測量 SVM 產生的 NFS 和 SMB 事件速率來確定最佳 Agent 機器大小。Event Rate Checker 指令碼可協助您了解容量限制（每個 Agent 最多 50 個資料收集器），並確保您的 Agent 基礎架構能夠處理預期的事件量，以實現可靠的威脅偵測。
