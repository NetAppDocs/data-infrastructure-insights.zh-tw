...
* 有關工作負載安全性原則配置的全面最佳實務和建議，請參閱link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/DII_Storage_Workload_Security__Fpolicy_Best_Practice_and_Recommendations["知識庫文章：FPolicy最佳實踐"^]。
* 您必須使用以下兩種方法之一新增 SVM：
+
...
+
*** 您也可以使用具有至少 csrole 權限的角色的 AD 用戶，如下面的「關於權限的說明」部分所述。另請參閱link:https://docs.netapp.com/ontap-9/index.jsp?topic=%2Fcom.netapp.doc.pow-adm-auth-rbac%2FGUID-0DB65B04-71DB-43F4-9A0F-850C93C4896C.html["ONTAP 文件"]。
