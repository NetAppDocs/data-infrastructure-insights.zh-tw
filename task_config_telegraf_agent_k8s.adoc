...
NOTE: 1.2057.0 之前的 operator 版本不支援一鍵升級。請依照以下手動升級說明升級至最新版本。升級完成後，即可使用一鍵升級功能進行後續升級。
...
確定現有 Operator 是否存在 _AgentConfiguration_（如果您的命名空間不是預設的 _netapp-monitoring_、請替換為對應的命名空間）：

 kubectl -n netapp-monitoring get agentconfiguration netapp-ci-monitoring-configuration
如果存在 _AgentConfiguration_：
...
+
** 使用相同的叢集名稱。
** 下載最新的 Operator YAML 檔案後，在部署之前，將 _agent_backup.yaml_ 中找到的任何自訂項目移植到下載的 _operator-config.yaml_。
** 確保您<<using-a-custom-or-private-docker-repository,拉取最新的容器鏡像>>如果您使用自訂儲存庫。
...
若要確定已部署的 operator 版本是否支援 _AgentConfiguration_，請執行下列命令：
...
在部署 Kubernetes Monitoring Operator 之前，請先設定 _operator-config.yaml_ 中 _AgentConfiguration_ 的 proxy 區段。
...
....
編輯 _operator-config.yaml_ 中的 _AgentConfiguration_，以反映新的 docker repo 位置。為您的私人儲存庫建立新的 imagePullSecret，更多詳細資訊請參閱 _https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry/_
...
某些環境（例如 Proxy 儲存庫）需要 Data Infrastructure Insights docker 儲存庫的長期密碼。安裝時在 UI 中提供的密碼僅有效 24 小時。您可以使用 API Access Token 作為 docker 儲存庫密碼，而不使用該密碼。只要 API Access Token 有效，此密碼就會一直有效。您可以為此特定目的產生新的 API Access Token，或使用現有的 API Access Token。

link:task_config_telegraf_agent.html#manually-create-an-api-access-token["在此閱讀"] 以取得建立新 API 存取權杖的說明。

若要從下載的 _operator-secrets.yaml_ 檔案中提取現有的 API 存取令牌，使用者可以執行下列命令：

`grep '\.dockerconfigjson' operator-secrets.yaml |sed 's/.*\.dockerconfigjson: //g' |base64 -d |jq`

若要從正在執行的 operator 安裝中提取現有的 API 存取令牌，使用者可以執行以下命令：
...
如果您執行的是 OpenShift 4.6 或更高版本，則必須編輯 _operator-config.yaml_ 中的 _AgentConfiguration_ 以啟用 _runPrivileged_ 設定：
...
| 出現“ImagePullBackoff”錯誤 | 如果您擁有自訂或私人 Docker 倉庫，但尚未配置 Kubernetes Monitoring Operator 以正確識別它，則可能會出現這些錯誤。<<using-a-custom-or-private-docker-repository,閱讀更多>>有關配置自訂/私有倉庫的資訊。 
...
| 安裝操作員後，telegraf-ds pod 進入 CrashLoopBackOff，pod 日誌顯示「su: Authentication failed」。 | 編輯 _AgentConfiguration_ 中的 telegraf 區段，並將 _dockerMetricCollectionEnabled_ 設為 false。如需更多詳細資訊，請參閱 operator 的link:telegraf_agent_k8s_config_options.html["配置選項"]。... spec: ... telegraf: ...- name: docker run-mode: - DaemonSet substitutions: - key: DOCKER_UNIX_SOCK_PLACEHOLDER value: unix:///run/docker.sock ...... 
...
| 我看到來自 Telegraf 的類似以下內容的警告訊息：_W！  [inputs.diskio] 無法收集「vdc」的磁碟名稱：讀取 /dev/vdc 時發生錯誤：沒有此檔案或目錄_ | 對於 Kubernetes Monitoring Operator 而言，這些警告訊息是良性的，可以安全地忽略。或者，在 AgentConfiguration 中編輯 telegraf 區段，並將 _runDsPrivileged_ 設為 true。如需更多詳細資訊，請參閱 link:telegraf_agent_k8s_config_options.html["操作員的配置選項"]。 
...
| 我在 Fluent-bit Pod 中收到以下錯誤，且 Pod 無法啟動：026/01/12 20:20:32] [error] [sqldb] error=unable to open database file [2026/01/12 20:20:32] [error] [input:tail:tail.0] db: could not create 'in_tail_files' table [2026/01/12 20:20:32] [error] [input:tail:tail.0] could not open/create database [2026/01/12 20:20:32] [error] failed initialize input tail.0 [2026/01/12 20:20:32] [error] [engine] input initialization failed | 確保資料庫檔案所在的主機目錄具有適當的讀取 / 寫入權限。更具體地說、主機目錄應授予非 root 使用者讀取 / 寫入權限。預設的資料庫檔案位置為 /var/log/ 、除非被 fluent-bit-dbFile _agentconfiguration_ 選項覆寫。如果啟用 SELinux 、請嘗試將 fluent-bit-seLinuxOptionsType agentconfiguration 選項設為 'spc_t' 


| 我看到 Telegraf 返回類似如下的錯誤訊息，但 Telegraf 確實啟動並運行了：E! [inputs.kubernetes] 插件錯誤： https://<IP>>:<port>>/pods[] 返回 HTTP 狀態 403 Forbidden | 如果您執行的是低於 1.33 版本的 Kubernetes，則可能會出現這些訊息。telegraf 使用的「nodes/pods」RBAC 資源在這些版本中不存在。有關舊版本所需的 RBAC 資源，請參閱 operator-additional-permissions.yaml 檔案中「適用於 Kubernetes 版本 < 1.33」部分。 
|===
