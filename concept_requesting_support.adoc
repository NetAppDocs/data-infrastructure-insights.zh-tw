...
[role="lead"]
Data Infrastructure Insights 提供全面的支援選項，包括自助服務資源、社群論壇、文件和技術支援。您可以透過選取 *Help > Support* 來存取這些支援資源，並考慮啟動您的支援權益，以便透過網路工單和電話支援解除鎖定完整的技術協助。
...
強烈建議您勾選此核取方塊，允許 NetApp 支援工程師在支援工作階段期間存取您的 Data Infrastructure Insights 網站。這將使工程師能夠疑難排解問題並協助您快速解決問題。支援工程師在存取您的網站期間，將能夠執行所有管理員層級的功能。

當您的問題已解決或您的支援工作階段結束時，您可以取消選取該核取方塊。

您可以前往 link:https://www.netapp.com/support-and-training/["NetApp 支援網站"] 申請支援。您必須擁有作用中的 Data Infrastructure Insights 訂閱。
