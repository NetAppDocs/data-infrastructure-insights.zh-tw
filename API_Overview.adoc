...
* ASSETS 類型包含資產、查詢和搜尋 API。DII 資產屬性和指標有時被稱為「odata」。這些 API 通常透過 /rest/v1/asset 或 /rest/v1/query 存取。如需詳細資訊，請參閱<<api-documentation-swagger,API Swagger 文件>>。
+
...
* 資料收集類型用於檢索和管理資料收集器。
* DATA INGESTION 類型用於擷取和管理擷取資料和自訂指標，例如來自 Telegraf 代理程式的資料。這些有時被稱為「資料湖」或「湖」指標。這些 API 通常透過 /rest/v1/lake 存取。如需詳細資訊，請參閱 <<api-documentation-swagger,API Swagger 文件>>。
* LOG INGESTION 用於檢索和管理日誌數據


隨著時間的推移，可能會有更多類型和/或 API 可用。您可以在link:#api-documentation-swagger["API Swagger 文件"]。

請注意，使用者可存取的 API 類型還取決於他們在每個 Data Infrastructure Insights 功能集（可觀測性、工作負載安全性、報告）中擁有的link:concept_user_roles.html["使用者角色"]。
