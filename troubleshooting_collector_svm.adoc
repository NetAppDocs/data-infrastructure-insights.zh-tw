...
'''
*問題：* 在新增或升級 ONTAP SVM 資料收集器（工作負載安全）後，租戶顯示過度資源配置或訂閱違規警報。即使未新增任何新硬體，原始儲存容量也增加了。

*試試這個：*同一個儲存設備可能會被計數兩次——一次是由 Observability ONTAP 叢集收集器計數，另一次是由同一叢集上的 Workload Security SVM 收集器計數。

比較 Observability ONTAP 收集器和 Workload Security SVM 收集器所報告的原始儲存容量。對於每個 SVM 收集器，檢查其父叢集是否已由 Observability ONTAP 收集器監控。如果已由其監控，則只有叢集容量才應計入訂閱使用量。

確認近期是否發生了跨叢集的 SVM 移轉。SVM 移轉後，容量計量可能仍與先前的叢集關聯，直到 Workload Security 從收集器收到更新資訊。

確保已移轉 SVM 的 ONTAP SVM 資料收集器處於 RUNNING 狀態。如有必要，重新啟動收集器，以便 Workload Security 在移轉後能夠收集正確的叢集和容量計量。

如果叢集容量和 SVM 容量都出現在總容量中，則很可能是容量被重複計算了。這種情況通常發生在新增、升級、重新啟動 SVM 收集器或在叢集之間移轉 SVM 之後不久。

如果使用量仍然超過您的訂閱，請開啟 NetApp Support 案例，提供警報前後的訂閱使用情況，以及可觀測性和 SVM 收集器及其報告的原始儲存容量清單。

{空的}

'''
*問題*：一個或多個資料收集器顯示「已降級」而非「正在執行」，即使收集器服務已啟動。

*試試這個*：當節點上的一個或多個 FPolicy 連線中斷時，SWS 會報告「效能降級」。這並非停電——收集器仍在運作——但來自這些節點的稽核事件可能會遺失，直到連線恢復為止。

ONTAP 會從每個擁有 SVM 運作資料 LIF 的節點建立 FPolicy 控制通道連線。沒有本機資料 LIF 的節點無法連線。參見：

* link:https://docs.netapp.com/us-en/ontap/nas-audit/node-fpolicy-server-communication-process-concept.html["節點到外部 ONTAP FPolicy 伺服器的通訊程序"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/fpolicy-external-fpolicy-servers-concept.html["ONTAP FPolicy 如何與外部 FPolicy 伺服器協同運作"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/requirements-best-practices-fpolicy-concept.html["設定 ONTAP FPolicy 的需求、考量事項及最佳實務做法"^]


在使用者介面中，記下哪些節點已中斷連線，然後檢查這些節點是否具有 SVM 的資料 LIF。

* 列出的節點上沒有資料 LIF：忽略「降級」狀態。當本地資料 LIF 不存在時，ONTAP 可能會顯示 FPolicy 已中斷連線，原因為「沒有本地 LIF 可連線至 FPolicy 伺服器」；這是預期行為。請參閱 link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/Fpolicy_server_not_connected_due_to_no_local_lif_present_on_one_or_more_nodes["知識庫：由於「no local lif present」，Fpolicy 伺服器未連線"^]。未來的 SWS 版本將不再於此情況下將收集器標記為「降級」。
* 列出的節點上存在資料 LIF：調查 FPolicy 斷線原因（LIF 關閉/缺少 data-fpolicy-client、網路/防火牆或暫時重新啟動/容錯移轉）。
