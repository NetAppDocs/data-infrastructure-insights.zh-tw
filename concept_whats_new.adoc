...
低於 100 的分數表示有潛在問題；分數越低，資源越不健康。將資源依資料中心或區域分組，以獲得直覺、全面的健康評分，讓您可以輕鬆確定關鍵區域的優先順序並進行故障排除。
...
除了資料基礎架構 Insights 擷取單元之外，現在還支援下列作業系統link:concept_acquisition_unit_requirements.html["已支援"]：
...
DII 的 *VM Analyzer* 功能提供全面的端對端虛擬化環境視圖。 VMAnalyzer 透過對應從虛擬機器到儲存的整個路徑來簡化故障排除，使用戶能夠快速找出虛擬磁碟、虛擬機器、主機、資料儲存區、磁碟區和儲存中的效能問題。透過直覺的拓撲視圖、篩選功能和詳細的效能指標，管理員可以輕鬆識別瓶頸、分析流量不平衡並優化整體儲存效能。

image:vm_analyzer_example_with_panel.png["儲存系統的 VM Analyzer 拓撲圖"]
...
* Debian （ 64 位元） 12.
* Oracle Enterprise Linux（64位元）9.6
* Red Hat（64位）9.6
...
=== 全新的 Landing 頁面讓您隨時掌握最新動態

重新設計的 Landing Page 可在單一檢視畫面中顯示您所需的一切，包括摘要資料，效能圖表，關聯性，警示和變更，以及重要資源。

現在，您可以更快進行疑難排解並保持順暢，而無需在索引標籤之間跳轉或失去內容。

image:lp_new_design.png["新的登陸頁設計，顯示摘要和效能的放置位置，以及其他資源區段"]



=== 工作負載安全性改善

* Webhooks 現在可用於工作負載安全警示 *

工作負載安全功能現在支援立即可用的 Webhook 通知，提供如 Slack ， PagerDuty ， Teams 等工具。此外，我們也提供可自訂的範本，可讓您量身打造訊息，或與任何其他 SIEM 或協力廠商應用程式整合。將關鍵警示直接路由至現有的安全工作流程，讓您的團隊能夠更快調查及回應。

image:ws_webhook_slack_example.png["Webhook Slack 工作負載安全性範例，寬度 =400"]

* 跨代理程式移轉工作負載安全收集器 *

您可以輕鬆地將工作負載安全收集器從一個代理程式移轉到另一個代理程式，以便在代理程式之間有效地平衡收集器的負載。移轉就像編輯收集器，並從清單中選取目的地代理程式一樣簡單。

image:ws_migrate_collector_to_another_agent.png["移轉收集器，寬度 = 500"]



=== CSV 非同步匯出

將資料匯出至 .CSV 的時間可能從數秒到數小時不等，視匯出的資料量而定。Data Infrastructure Insights 現在會以非同步方式匯出該資料，以便您在編譯 .CSV 時繼續工作。

選取右上角工具列中的「鈴聲」圖示，即可檢視 .CSV 匯出。

image:csv_export_async.png["鐘形圖示，其中包含可供下載的 .csv 匯出清單，寬度 = 400"]
