...
隆重推出我們最新的功能，旨在增強您的安全性與資源管理功能！利用進階的鑑識群組依據功能和階層式的多個群組依據支援，您現在可以輕鬆識別哪些使用者存取特定資料夾，判斷最活躍的使用者和共用，並追蹤作用中的用戶端 IP 位址，主動管理風險。找出最容易存取的檔案和資料夾，以最佳化儲存和頻寬使用率，並透過識別真實使用者來加強對系統存取的控制。

image:forensics-group-by-example-screen.png["取證範例畫面顯示分組依據"]
