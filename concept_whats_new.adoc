...
將複雜資料即時轉化為圖形化洞察。只需讓 AI 助理建立儀表板，它就能以互動式圖表和表格的形式呈現結果，幫助您更快地識別趨勢並解決問題，而無需手動建立圖表。

image:ai_assistant-column_graph.png["AI Assistant 視覺化"]
...
VM Analyzer 檢視中的新切換功能可讓您隱藏沒有資料儲存區的已掛載磁碟區，提供更清晰的檢視，簡化 VM 疑難排解。

image:vm_analyzer_datastore_only_view.png["僅資料儲存檢視滑桿"]
...
在確保資產資訊準確性的同時，降低意外變更的風險。註釋和屬性（即「User Data」）預設以唯讀模式顯示，因此您可以先審查詳細資訊，然後再決定何時進行編輯。

僅在需要時進行編輯。從側拉式面板中展開資產詳細資料，然後選擇鉛筆圖示。資產活動網頁會以編輯模式開啟，您可以在其中更新資訊，並在完成後選擇「儲存」。

您也可以直接開啟資產活動網頁，然後選擇右上角的「編輯」。

image:UserDataEdit-Slideout.png["在側邊欄面板中編輯使用者資料 - 鉛筆圖示，寬度=400"]

image:UserDataEdit-LP.png["活動網頁編輯模式，寬度=200"]
...
DII AI 助理現在擁有專用的起始頁面，將至能基礎架構分析置於核心位置。您可以立即開始新的對話，一鍵恢復上次工作階段，或透過建議提示快速入門。

image:AI_dashboard_page_default.png["AI 儀表板預設，寬度=800"]

您可以透過導覽功能表中的「AI Assistant」存取起始頁面，或使用左上角的「星形」圖示從任意位置開啟 AI Assistant。

image:AI_dashboard_page_menu.png["AI Assistant Nav Menu, width=600"] image:AI_assistant_access_button.png["隨時隨地存取 AI 助手，寬度=300"]
...
透過可自訂的登入訊息，讓您的團隊隨時了解最新動態並保持一致。在使用者登入時，直接向他們傳達重要公告、維護計劃或策略更新。透過 Markdown 支援，您可以格式化訊息，使其更加清晰易懂，並確保關鍵訊息能夠傳達給每位 DII 使用者。擁有管理員權限的帳戶可以在「管理」功能表的「通知」下找到相關配置。
...
DII 導覽功能表煥然一新，現在與 Console 的色彩和行為相符，提供更統一的體驗。先前的靜態主功能表現在預設為摺疊狀態，以提供更專注的工作區，並有效利用螢幕空間。只需按一下左上角的漢堡圖示（☰）即可存取。
...
透過 AI 驅動的裝置識別功能，加速新裝置的上線。Data Infrastructure Insights 會分析您的環境並建議自動裝置解析規則，協助您快速識別和管理更多裝置。您只需審核並核准 AI 產生的規則，DII 即可處理其餘工作，從而提升整個基礎架構的裝置可見度和管理效率。
...
AI 助理現在支援十幾種語言,讓您的團隊能夠用他們偏好的語言進行自然交流。查看完整清單link:https://platform.claude.com/docs/en/build-with-claude/multilingual-support["這裡"^]。
...
現在可以使用 Attribute Monitors，在資產於資料中心之間移動、儲存資源池未加密或發生任何屬性修改時自動向您的團隊發出警報。這有助於維護組態基準，並確保在新部署或變更期間不會出現任何疏漏。

image:attribute_monitor_in_list.png["屬性監視器已新增至監視器類型"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== 利用動態、富含上下文的訊息建立更聰明的警報和 webhook

建立監控或 webhook 時，只需在警示說明或 webhook URL、標頭或內文中輸入「%」，即可從動態填入的可用物件屬性和註釋清單中進行選擇。為您的警示提供正確的內容，以提高清晰度、增強 webhook 整合並加快回應速度。

image:attribute_list_example.png["監視器描述中的屬性清單，width=400"]
...
=== Data Infrastructure Insights 比以往任何時候都更了解您的環境

我們增強了 Data Infrastructure Insights 中的 AI 對您設定的理解能力。它會在回應之前，利用更豐富的上下文資訊，例如您目前對服務的使用情況、組態以及效能模式。這樣一來，它就能提供更貼合您實際環境的初始指導，而非通用的起點，從而減少後續追蹤，讓您更快獲得有用的答案。

例如，導覽至儲存裝置後， AI 助理會一鍵回答諸如以下問題：

* 此儲存設備的容量使用率趨勢如何，以及哪些 StoragePools 佔用最多空間？
* 您能否顯示此裝置在過去 24 小時內的容量使用率趨勢？
* 此儲存設備上哪些磁碟區佔用的空間最多？
...
Data Infrastructure Insights AI Assistant 現已對所有在 AWS 上託管 DII 站台的客戶全面開放。

請注意，Azure 網站上的 AI Assistant 目前仍處於 Preview 階段。請關注後續的正式發佈公告。



[[infrastructure-insights-features-removed]]
=== 已移除的 Infrastructure Insights 功能

DII 的「Infrastructure Insights」功能，包括「共享資源壓力」、「Kubernetes 命名空間空間不足」和「回收 ONTAP 冷存儲」，已不再在 DII 中提供。
...
隆重介紹您基礎架構的全新智慧夥伴：link:ai_for_dii_overview.html["*Data Infrastructure Insights AI 助理*"] 徹底改變您與資料的互動方式，並將複雜的查詢轉化為簡單的對話。即時發現隱藏模式，加速故障排除，並自信地做出數據驅動的決策。立即取得以下問題的答案：
...
DII AI Assistant Preview 現已開放給大多數在 AWS 區域託管服務的客戶；Azure 區域即將推出。link:concept_preview_features.html["了解更多"] 關於 Data Infrastructure Insights 預覽功能。

image:ai_assistant_icon.png["AI 助理圖示"]
...
Reclaim ONTAP Cold Storage Insight 現已支援 FlexGroups，並且現已向所有客戶開放。
...
| Kubernetes 命名空間空間不足 | 「Kubernetes Namespaces Running Out of Space」Insight 可讓您了解 Kubernetes 命名空間中哪些工作負載有空間不足的風險，並估算每個空間用完之前剩餘的天數。 


| 共享資源面臨壓力 | _Shared Resource Under Stress_ 洞察功能利用 AI/ML 自動識別環境中資源爭用導致效能下降的位置，突出顯示受其影響的任何工作負載，並提供建議的補救措施，讓您更快地解決效能問題。 
...
Cloud Insights 會定期推出許多令人興奮的全新預覽功能。如果您有興趣預覽其中一項或多項功能，請聯絡您的 NetApp 銷售團隊以了解更多資訊。
...
| 共享資源面臨壓力 | 「共享資源壓力」洞察功能利用 AI/ML 自動識別環境中資源爭用導致效能下降的位置，突出顯示受其影響的任何工作負載，並提供建議的補救措施，讓您更快地解決效能問題。 
|===
