...
Data Infrastructure Insights AI 助理現已推出預覽版，並包含以下增強功能：

* Azure 區域可用性：對於 DII 網站託管在 Azure 上的客戶，Data Infrastructure Insights 助理現在已準備好為您提供協助。
* 最多可取得 7 天的資料：Assistant 現在可以存取最多 7 天的高解析度指標，從而實現更廣泛的資料分析和洞察。
