...
強化 REST API 使用率追蹤功能，強化您的安全狀態，並簡化資源管理。透過 API 使用率追蹤，您可以查看使用的 API Token ， IP 位址及其產生的流量量。這項資訊可供管理員使用者存取，可讓您快速偵測異常活動模式，識別潛在威脅，並透過消除不必要的要求來最佳化效能。將權杖連結至特定的 IP 位址和使用層級，即可深入瞭解系統存取和使用趨勢，讓您掌控維持安全，有效率的環境，並讓作業順暢運作。

若要檢視 API 使用率，請瀏覽至 * 可服務性 > 管理 > API 存取 * ，然後選取 _ 檢視 API 使用率 _ 。請注意，此 API 僅適用於 DII 可服務性 API ；不適用於工作負載安全性或報告 API 。
