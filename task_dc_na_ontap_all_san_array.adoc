...
---
= NetApp ONTAP All SAN Array （ ASA ）資料收集器
:hardbreaks:
:toclevels: 2
:allow-uri-read: 
:toclevels: 2
...
[role="lead"]
此資料收集器會使用 REST API 呼叫、從執行 ONTAP 9.16.0 及更高版本的儲存系統擷取庫存、 EMS 記錄和效能資料。
...
NOTE: ASA R2 代表最新一代的 ONTAP ASA 儲存平台。這包括陣列型號 ASA A1K ， A90 ， A70 ， A50 ， A30 和 A20 。

對於所有前一代 ASA 系統，請使用link:task_dc_na_ontap_rest.html["ONTAP REST"]收集器。
